...
GRV.iconOctoCat = `<img src="https://www.ladybug.tools/spider-2020/assets/icons/octicon.svg">`;

GRV.link = `<img width=16 src="https://www.ladybug.tools/spider-2020/assets/icons/icon-link-external.svg">`;
...
GRV.onLoadTree = function (json) {
	...
	const filesRoot = files
		...
		<div>
			...
			<a href="https://www.ladybug.tools/spider-2020/${item}"title="Link to just this file. Open file in new tab."  target="_blank" >${ GRV.link }</a>
		</div>`);
...
GRV.getFiles = function (subtree, files) {
	...
	const filtered = files
      ...
      ${ GRV.iconOctoCat }</a>
			...
			<a href="https://www.ladybug.tools/spider-2020/${item}" title="Open file in new tab"  target="_blank" >
			${ GRV.link }</a>
		</div>`);
