...
GRV.iconOctoCat = `<img src="./assets/icons/octicon.svg">`;

GRV.link = `<img width=16 src="./assets/icons/icon-link-external.svg">`;
...
GRV.onLoadTree = function (json) {
	...
	const filesRoot = files
		...
		.map( ( item, i ) => `
		<div>
			<a href="https://github.com/theo-armour/2020/tree/master/${item}"  title="Source code on GitHub. Edit me!" >
			${ GRV.iconOctoCat }</a>
			${ i + 1 } <a href="#${item}" >${item.split("/").pop()}</a>
			<a href="https://theo-armour.github.io/2020/${item}"title="Link to just this file. Open file in new tab."  target="_blank" >${ GRV.link }</a>
		</div>`);
...
GRV.getFiles = function (subtree, files) {
	...
	const filtered = files
		...
		.map(item => `
		<div>
			<a href="https://github.com/theo-armour/2020/tree/master/${item}" title="Source code on GitHub" >
      ${ GRV.iconOctoCat }</a>
			<a href="#${item}" title="">${item.split("/").pop()}</a>
			<a href="https://theo-armour.github.io/2020/${item}" title="Open file in new tab"  target="_blank" >
			${ GRV.link }</a>
		</div>`);
