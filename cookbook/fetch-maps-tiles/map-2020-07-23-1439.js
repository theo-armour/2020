...
MAP.getTilesHeightMaps = function( onLoadHeightMaps ) {
	...
	MAP.zoomHeightMap = MAP.zoom;

	tileCenterX = MAP.lonToTile( MAP.longitude, MAP.zoomHeightMap );
	tileCenterY = MAP.latToTile( MAP.latitude, MAP.zoomHeightMap );

	tileStartX = tileCenterX - Math.floor( MAP.cols / 2 );
	tileStartY = tileCenterY - Math.floor( MAP.rows / 2 );
	...
	for ( let i = 0; i < MAP.cols; i++ ) {
		...
		for ( let j = 0; j < MAP.rows; j++ ) {
			...
			const url = `https://api.mapbox.com/v4/mapbox.terrain-rgb/${ MAP.zoomHeightMap }/${ MAP.tileStartX + i }/${ tileStartY + j }.pngraw?access_token=${ MAP.mbptoken}`;
...
MAP.onLoadTileHeightMap = function ( src, col = 0, row = 0 ) {
	...
	img.onload = function () {
		...
		//MAP.contextHeightMaps.drawImage( img, 0, 0, MAP.pixelsPerTile, MAP.pixelsPerTile);
